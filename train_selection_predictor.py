...
def train_selection_predictor():
    ...
    for epoch in range(num_epochs):
        ...
        val_selection_loss, val_color_loss, val_color_acc = evaluate_selection_and_color(
            selection_mask_predictor, color_predictor, state_encoder, mask_encoder, 
            val_loader, device, color_criterion, num_color_selection_fns, num_selection_fns, use_vicreg, vicreg_loss_fn
        )
        print(f"Epoch {epoch+1}/{num_epochs} - Train Color Loss: {avg_color_loss:.4f} | Train Selection Loss: {avg_selection_loss:.4f}")
        print(f"Val Color Loss: {val_color_loss:.4f} | Val Selection Loss: {val_selection_loss:.4f} | Val Color Acc: {val_color_acc:.4f}")

#!/usr/bin/env python3
"""
Test script to verify that embedded actions work correctly with both color and mask predictors.
"""
...
def test_embedded_actions():
    """Test that embedded actions work correctly with both predictors."""
    ...
    print("Testing embedded actions with color and mask predictors...")
    ...
    # Create dummy data
    # State encoder expects values in [-1, vocab_size-2] where -1 is padding
    ...
    # Mask encoder also expects values in [-1, vocab_size-2] where -1 is padding
    ...
    selection_loss = torch.nn.MSELoss()(pred_latent_mask, target_latent_mask)
    
    print(f"\nLosses:")
    print(f"  Color loss: {color_loss.item():.4f}")
    print(f"  Selection loss: {selection_loss.item():.4f}")
    
    print("\n✅ All tests passed! Embedded actions work correctly.")

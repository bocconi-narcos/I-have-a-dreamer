...
class RewardPredictor(nn.Module):
    ...
    Takes three latent vectors (z_t, z_{t+1}, z_target), performs attention among them,
    ...
    Input:
        z_t: (B, D) - current state encoding
        z_tp1: (B, D) - next state encoding  
        z_target: (B, D) - target state encoding
    Output:
        (B, 1) scalar reward
    """
    def __init__(self, latent_dim, hidden_dim=128, transformer_depth=2, transformer_heads=2, transformer_dim_head=64, transformer_mlp_dim=128, dropout=0.1, proj_dim=None):
        ...
        # Transformer with input normalization
    ...
    def forward(self, z_t, z_tp1, z_target):
        ...
        x = self.input_norm(x)  # Normalize before transformer
        
        # Apply transformer
        x = self.transformer(x)  # (B, 4, D)
        
        # Extract CLS token and predict reward
        cls_out = x[:, 0]  # (B, D)

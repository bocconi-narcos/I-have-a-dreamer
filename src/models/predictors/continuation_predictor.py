...
class ContinuationPredictor(nn.Module):
    """
    Transformer-based continuation predictor.
    Takes two latent vectors (z_t, z_{t+1}), performs attention among them (seq=2),
    prepends a CLS token, adds positional encoding, and outputs a probability (boolean) of continuation.
    Args:
    ...
    Input:
        ...
        z_tp1: (B, D)
    Output:
        (B, 1) probability of continuation
    """
    ...
    def forward(self, z_t, z_tp1):
        ...
        cls_out = x[:, 0]  # (B, D)
        cont = self.mlp_head(cls_out)  # (B, 1)
        return self.sigmoid(cont) 